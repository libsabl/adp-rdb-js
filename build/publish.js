...
async function addReadmeHeaders(source, pkgv) {
  ...
  const codeCovImg = `https://codecov.io/gh/libsabl/adp-rdb-js/commit/${commit}/graph/badge.svg?token=TVL1XYSJHA`;
  const codeCovPage = `https://codecov.io/gh/libsabl/adp-rdb-js/tree/${commit}/src`;
  const ghBrowse = `https://github.com/libsabl/adp-rdb-js/tree/${commit}`;
  let docsPath = `https://github.com/libsabl/adp-rdb-js/blob/${commit}/docs/DOCS.md`;

  let md = `[![codecov](${codeCovImg})](${codeCovPage})`;
  md += EOL + EOL + '**version**: `' + pkgv + '`';

  if (pkgv.match(/^\d+\.\d+\.\d+$/)) {
    docsPath = `https://github.com/libsabl/adp-rdb-js/blob/v${pkgv}/docs/DOCS.md`;
    const ghRelease = `https://github.com/libsabl/adp-rdb-js/releases/tag/v${pkgv}`;
